...
public class ClassModifier {
    ...
    public byte[] modifyUTF8Constant(String oldStr, String newStr) {
        ...
        int offset = CONSTANT_POOL_COUNT_INDEX + u2;  // 真实的常量起始位置，常量池前两个字节表示常量池中常量的数量 cpc
        for (int i = 1; i < cpc; i++) {
            int tag = ByteUtils.byte2Int(classByte, offset, u1);
            if (tag == CONSTANT_UTF8_INFO) {
                int len = ByteUtils.byte2Int(classByte, offset + u1, u2);
                offset += u1 + u2;
                String str = ByteUtils.byte2String(classByte, offset, len);//表内容
                if (str.equals(oldStr)) {
                    byte[] strReplaceBytes = ByteUtils.string2Byte(newStr);
                    byte[] intReplaceBytes = ByteUtils.int2Byte(strReplaceBytes.length, u2);//将长度转化为字节数组表示的形式
                    // 替换新的字符串的长度
